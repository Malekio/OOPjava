...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username=demo_guide_username).exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created demo guide: {demo_guide_username}"
                )
            )
        else:
            self.stdout.write(
        ...
        if not User.objects.filter(username=demo_tourist_username).exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created demo tourist: {demo_tourist_username}"
                )
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"Demo tourist {demo_tourist_username} already exists"
                )
            )
