...
class Review(models.Model):
    ...
    tourist = models.ForeignKey(
        TouristProfile, on_delete=models.CASCADE, related_name="reviews_given"
    )
    guide = models.ForeignKey(
        GuideProfile, on_delete=models.CASCADE, related_name="reviews_received"
    )
    ...
    def clean(self):
        ...
        if self.booking.status != "completed":
            raise ValidationError({"booking": "Can only review completed bookings."})

        if self.booking.tourist != self.tourist:
            raise ValidationError({"tourist": "Can only review your own bookings."})
    ...
    def update_guide_rating(self):
        ...
        guide_stats = Review.objects.filter(
            guide=self.guide, is_approved=True
