...
from .serializers import (
    ...
    ReviewListSerializer, GuideResponseSerializer
)
...
class ReviewDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not hasattr(request.user, 'tourist_profile') or review.tourist != request.user.tourist_profile:
            return Response(
...
def guide_respond_to_review(request, review_id):
    ...
    review = get_object_or_404(
        Review.objects.select_related('guide'),
        id=review_id
    )
