...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Review
        fields = [
            'id', 'tourist', 'guide', 'tour', 'booking',
            'tourist_name', 'guide_name', 'tour_title',
            'rating', 'title', 'comment', 'can_edit',
            'is_approved', 'is_featured', 'guide_response',
            'guide_responded_at', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'tourist', 'guide', 'tour', 'booking',
            'is_approved', 'is_featured', 'guide_response',
    ...
    def get_can_edit(self, obj):
        ...
        if request and request.user.is_authenticated:
            return (hasattr(request.user, 'tourist_profile') and 
                   obj.tourist == request.user.tourist_profile)
        return False
...
class GuideResponseSerializer(serializers.ModelSerializer):
    ...
    def validate_guide_response(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError(
                "Guide response cannot be empty."
            )
        return value.strip()
...
class ReviewListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Review
        fields = [
            'id', 'tourist_name', 'guide_name', 'tour_title',
            'tourist_avatar', 'rating', 'title', 'comment',
            'guide_response', 'created_at'
        ]
