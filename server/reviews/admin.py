...
class ReviewAdmin(admin.ModelAdmin):
    ...
    list_display = ['title', 'tourist', 'tour_title', 'guide', 'rating', 'rating_stars', 
                    'is_approved', 'has_response', 'created_at']
    list_filter = ['rating', 'is_approved', 'created_at', 'tour__wilaya']
    search_fields = ['title', 'comment', 'tourist__user__username', 'tourist__user__first_name', 
    ...
    fieldsets = (
        ('Review Information', {
            'fields': ('tourist', 'tour', 'booking')
        }),
        ('Review Content', {
            'fields': ('rating', 'rating_stars', 'title', 'comment')
        }),
        ('Guide Response', {
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        avg_rating = queryset.filter(is_approved=True).aggregate(Avg('rating'))['rating__avg'] or 0
        reviews_with_response = queryset.exclude(guide_response__isnull=True).exclude(guide_response='').count()
        ...
        extra_context['review_stats'] = {
            ...
            'response_rate': round((reviews_with_response / total_reviews * 100) if total_reviews > 0 else 0, 1),
        }
