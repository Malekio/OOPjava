...
from .serializers import (
    ...
    CustomTourRequestSerializer, CustomTourRequestCreateSerializer
)
...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if not hasattr(request.user, 'tourist_profile'):
            return Response({'error': 'Only tourists can start conversations'}, 
    ...
    def send_message(self, request, pk=None):
        ...
        if hasattr(user, 'tourist_profile') and conversation.tourist == user.tourist_profile:
            sender_type = 'tourist'
        elif hasattr(user, 'guide_profile') and conversation.guide == user.guide_profile:
            sender_type = 'guide'
        else:
            return Response({'error': 'Not authorized for this conversation'}, 
                          status=status.HTTP_403_FORBIDDEN)
    ...
    def mark_read(self, request, pk=None):
        ...
        if hasattr(user, 'tourist_profile'):
            conversation.messages.filter(sender_type='guide', is_read=False).update(is_read=True)
        elif hasattr(user, 'guide_profile'):
            conversation.messages.filter(sender_type='tourist', is_read=False).update(is_read=True)
...
class CustomTourRequestViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        """
        if not hasattr(request.user, 'tourist_profile'):
            return Response({'error': 'Only tourists can create custom tour requests'}, 
                          status=status.HTTP_403_FORBIDDEN)
    ...
    def respond(self, request, pk=None):
        ...
        if not hasattr(request.user, 'guide_profile') or custom_request.guide != request.user.guide_profile:
