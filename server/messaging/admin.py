...
class ConversationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Participants", {"fields": ("tourist", "guide")}),
        ("Conversation Details", {"fields": ("subject",)}),
        (
            "Statistics",
    ...
    def get_queryset(self, request):
        ...
        return (
            ...
            .select_related("tourist__user", "guide__user")
...
class MessageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Message Details", {"fields": ("conversation", "sender_type", "content")}),
    ...
    def get_queryset(self, request):
        ...
        return (
            super()
            .get_queryset(request)
            .select_related("conversation__tourist__user", "conversation__guide__user")
        )
...
class CustomTourRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Request Details", {"fields": ("tourist", "guide", "title", "description")}),
        (
            "Tour Specifications",
            {"fields": ("preferred_date", "duration_hours", "group_size", "budget")},
        ),
        ("Special Requirements", {"fields": ("special_requirements",)}),
        (
            "Guide Response",
            {
                "fields": (
    ...
    def get_queryset(self, request):
        """Optimize queryset with related data"""
        return (
            super().get_queryset(request).select_related("tourist__user", "guide__user")
        )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["request_stats"] = {
            ...
            "acceptance_rate": round(
                (accepted_requests / total_requests * 100) if total_requests > 0 else 0,
                1,
            ),
        }
