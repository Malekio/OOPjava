...
class BookingSerializer(serializers.ModelSerializer):
    ...
    def validate_group_size(self, value):
        ...
        if value <= 0:
            raise serializers.ValidationError("Group size must be greater than 0.")
        return value
...
class BookingCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Booking
        fields = ["tour", "booking_date", "time_slot", "group_size", "notes"]
...
class BookingStatusUpdateSerializer(serializers.ModelSerializer):
    ...
    action = serializers.ChoiceField(
        choices=["confirm", "reject", "complete"], write_only=True
    )
    ...
    def validate_alternative_date(self, value):
        ...
        if value and value <= timezone.now().date():
            raise serializers.ValidationError("Alternative date must be in the future.")
        return value
...
class BookingListSerializer(serializers.ModelSerializer):
    ...
    guide_name = serializers.CharField(
        source="tour.guide.user.get_full_name", read_only=True
    )
    days_until_booking = serializers.SerializerMethodField()
