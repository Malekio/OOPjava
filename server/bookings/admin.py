...
class BookingAdmin(admin.ModelAdmin):
    ...
    list_display = ['id', 'tourist', 'tour_title', 'guide', 'booking_date', 
                    'time_slot', 'status', 'total_price', 'group_size', 'created_at']
    list_filter = ['status', 'time_slot', 'booking_date', 'created_at', 'tour__wilaya']
    search_fields = ['tourist__user__username', 'tourist__user__first_name', 
                     'tourist__user__last_name', 'tour__title', 'tour__guide__user__username']
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).select_related(
            'tourist__user', 'tour__guide__user', 'tour__wilaya'
        )
    
    actions = ['confirm_bookings', 'reject_bookings', 'cancel_bookings', 'complete_bookings']
    ...
    def reject_bookings(self, request, queryset):
        ...
        updated = queryset.filter(status__in=['pending', 'confirmed']).update(status='rejected')
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        confirmed_bookings = self.get_queryset(request).filter(status='confirmed').count()
        completed_bookings = self.get_queryset(request).filter(status='completed').count()
